...
class DBDataUtil
{
    ...
    public static function transformAnimationData(animationData:AnimationData, armatureData:ArmatureData, isGlobalData:Bool):Void
    {
        ...
        for (i in 0...boneDataList.length){
            ...
            for (j in 0...frameListLength){
                var frame:TransformFrame = try cast(frameList[j], TransformFrame) catch(e:Dynamic) null;
    ...
    static function setFrameTransform(animationData:AnimationData, armatureData:ArmatureData, boneData:BoneData, frame:TransformFrame):Void
    {
    ...
    static function getTimelineTransform(timeline:TransformTimeline, position:Int, retult:DBTransform, isGlobal:Bool):Void
    {
        var frameList:Array<Frame> = timeline.frameList;
        ...
        while (i-->0)
        {
            var currentFrame:TransformFrame = try cast(frameList[i], TransformFrame) catch(e:Dynamic) null;
            if (currentFrame.position <= position && currentFrame.position + currentFrame.duration > position) 
            {
                ...
                else 
                {
                    ...
                    var nextFrame:TransformFrame = try cast(frameList[i + 1], TransformFrame) catch(e:Dynamic) null;
