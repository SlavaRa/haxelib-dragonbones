...
class NativeTextureAtlas implements ITextureAtlas
{
    ...
    public function new(texture:Dynamic, textureAtlasRawData:Dynamic, textureScale:Float = 1, isDifferentConfig:Bool = false)
    {
        ...
        {
            _bitmapData = try cast(texture, BitmapData) catch(e:Dynamic) null;
        }
        else if (Std.is(texture, MovieClip)) 
        {
            _movieClip = try cast(texture, MovieClip) catch(e:Dynamic) null;
            _movieClip.stop();
        }
    ...
    {
        var textureData:TextureData = try cast(Reflect.field(_subTextureDataDic, name), TextureData) catch(e:Dynamic) null;
        if (textureData != null) 
        {
